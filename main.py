...
class MainWidget(RelativeLayout):
    ...
    from users_actions import keyboard_closed, on_keyboard_up, on_keyboard_down, on_touch_up, on_touch_down
    menu_widget = ObjectProperty()
    perspective_point_x = NumericProperty(0)
    perspective_point_y = NumericProperty(0)

    V_NB_LINES = 8
    V_LINES_SPACING = .4  # percentage width of the screen
    vertical_lines = []

    H_NB_LINES = 15
    H_LINES_SPACING = .1  # percentage height of the screen
    horizontal_lines = []
    ...
    def is_desktop(self):
        if platform in ('linus', 'windows', 'macosx'):
            return True
        return False
    ...
    def get_line_x_from_index(self, index):
        center_lines_x = self.perspective_point_x
        spacing = self.V_LINES_SPACING * self.width
        offset = index - 0.5
        line_x = center_lines_x + offset * spacing + self.current_offset_x
        return line_x
